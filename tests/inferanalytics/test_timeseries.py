...
from inferanalytics.utils import to_utc, to_series, pandas_one_point_one_or_less
...
class TestDrawdown(TestCase):
    ...
    @parameterized.expand([(drawdown_serie,)])
...
class TestStats(TestCase):
    simple_rets = pd.Series(
        [0.1] * 3 + [0] * 497, pd.date_range("2000-1-3", periods=500, freq="D")
    )

    simple_week_rets = pd.Series(
        [0.1] * 3 + [0] * 497,
        pd.date_range("2000-1-31", periods=500, freq="W"),
    )

    simple_month_rets = pd.Series(
        [0.1] * 3 + [0] * 497,
        pd.date_range("2000-1-31", periods=500, freq="M"),
    )

    simple_benchmark = pd.Series(
        [0.03] * 4 + [0] * 496,
        pd.date_range("2000-1-1", periods=500, freq="D"),
    )
    px_list = np.array([10, -10, 10]) / 100.0  # Ends in drawdown
    dt = pd.date_range("2000-1-3", periods=3, freq="D")
...
class TestGrossLev(TestCase):
    ...
    test_gross_lev = pd.read_csv(
        gzip.open(__location__ + "/test_data/test_gross_lev.csv.gz"),
        index_col=0,
        parse_dates=True,
    )
