...
def simple_moving_average(
    ...
    df_with_signal["signal"] = sma_indicator(df_with_signal["close"], 
                                             window=window)
...
def weighted_moving_average(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = wma_indicator(df_with_signal["close"], 
                                             window=window)
    return df_with_signal
...
def exponentially_weighted_moving_average(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = ema_indicator(df["close"], 
                                             window=window, 
                                             fillna=True)
    return df_with_signal
...
def moving_average_convergence_divergence(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = macd_signal(df["close"], 
                                           window_slow, 
                                           window_fast, 
                                           window_signal, 
                                           fillna=True)
...
def relative_strength_index(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = rsi(df["close"], 
                                   window=window, 
                                   fillna=True)
    return df_with_signal
...
def stochastic_relative_strength_index(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = stochrsi(df["close"], 
                                        window=window, 
                                        fillna=True)
    return df_with_signal
...
def chande_kroll(
    ...
    df.drop(columns=PandasEnum.SIGNAL.value, 
            inplace=True)
...
def bollinger_band(
    ...
    df_with_signal["BOLU"] = bollinger_hband(df_with_signal["typical_price"], 
                                             window=window, 
                                             window_dev=window_dev)
    df_with_signal["BOLD"] = bollinger_lband(df_with_signal["typical_price"], 
                                             window=window, 
                                             window_dev=window_dev)
    df_with_signal["BOLA"] = bollinger_mavg(df_with_signal["typical_price"], 
                                            window=window)
...
def detrended_price_oscillator(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = dpo(df["close"], 
                                   window=window)
    return df_with_signal
...
def percentage_price_oscillator(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = ppo_signal(df["close"], 
                                          window_slow, 
                                          window_fast, 
                                          window_signal, 
                                          fillna=True)
...
def percentage_volume_oscillator(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = pvo_signal(df["volume"], 
                                          window_slow, 
                                          window_fast, 
                                          window_signal, 
                                          fillna=True)
...
def triple_exponential_average(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = trix(df["close"], 
                                    window, 
                                    fillna=True)
    return df_with_signal
...
def true_strength_index(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["TSI"] = tsi(df["close"], 
                                window_slow, 
                                window_fast, 
                                fillna=True)
    df_with_signal["signal"] = ema_indicator(df_with_signal["TSI"], 
                                             window_signal, 
                                             fillna=True)
    return df_with_signal
...
def schaff_trend_cycle(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = stc(df_with_signal["close"], 
                                   window_slow, 
                                   window_fast, 
                                   cycle, 
                                   smooth1, 
...
def KAMA(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = kama(df["close"], 
                                    window, 
                                    pow1, 
                                    pow2)
    return df_with_signal
...
def aroon(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["aroon_up"] = aroon_up(df["close"], 
                                          window, 
                                          fillna=True)
    df_with_signal["aroon_down"] = aroon_down(df["close"], 
                                              window, 
                                              fillna=True)
    return df_with_signal
...
def rate_of_change(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["signal"] = roc(df["close"], 
                                   window, 
                                   fillna=True)
    return df_with_signal
...
def average_directional_movement_index(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["ADX_POS"] = adx_pos(df["high"], 
                                        df["low"], 
                                        df["close"], 
                                        window, 
                                        fillna=True)
    df_with_signal["ADX_NEG"] = adx_neg(df["high"], 
                                        df["low"], 
                                        df["close"], 
                                        window, 
                                        fillna=True)
    df_with_signal["ADX"] = adx(df["high"], 
                                df["low"], 
                                df["close"], 
                                window, 
                                fillna=True)
...
def vortex_indicator(
    ...
    """
    df_with_signal = df.copy()
    df_with_signal["VORTEX_POS"] = vortex_indicator_pos(df["high"], 
                                                        df["low"], 
                                                        df["close"], 
                                                        window, 
                                                        fillna=True)
    df_with_signal["VORTEX_NEG"] = vortex_indicator_neg(df["high"], 
                                                        df["low"], 
                                                        df["close"], 
                                                        window, 
                                                        fillna=True)
...
infertrade_export_signals = {
    "normalised_close": {
        ...
        "available_representation_types": {
            "github_permalink": "https://github.com/ta-oliver/infertrade/blob/e49334559ac5707db0b2261bd47cd73504a68557/infertrade/"
    ...
    "chande_kroll": {
        ...
        "parameters": {
            "average_true_range_periods": 10, 
            "average_true_range_multiplier": 1.0, 
            "stop_periods": 9},
        "series": ["high", "low"],
        "available_representation_types": {
            "github_permalink": "https://github.com/ta-oliver/infertrade/blob/e49334559ac5707db0b2261bd47cd73504a68557/infertrade/"
    ...
    "bollinger_band": {
        ...
        "available_representation_types": {
            "github_permalink": "https://github.com/ta-oliver/infertrade/blob/5f74bdeb99eb26c15df0b5417de837466cefaee1/infertrade/"
